...
var EvaluationProduct = require('../models/evaluateproduct')
// GET Category index
router.get('/',isAdmin,function(req, res){
  ...
  router.get('/hidden/:id', isAdmin, function (req, res) {
    ...
    EvaluationProduct.findOne({_id:idBlock},function(err,u){
        u.status = "Đã ẩn"
        u.checkhidden = true;
        u.save();
        req.flash('Thành công:', `Đã ẩn bình luận thành công`);
  ...
  router.get('/unhidden/:id', isAdmin, function (req, res) {
    ...
    EvaluationProduct.findOne({_id:idBlock},function(err,u){
        ...
        u.checkhidden = false;
        u.save();
        req.flash('Thành công:', `Đã hiển thị bình luận của ${u.name}!`);
        res.redirect('/admin/comment');
    });
 });
